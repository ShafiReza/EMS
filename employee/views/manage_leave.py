...
class AddLeaveApplicationView(LoginRequiredMixin, EmployeePassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        applied_leave_days = (leave_to - leave_from).days + 1
        ...
        # Query for approved leaves in the current month
        approved_leaves = LeaveApplication.objects.filter(
            approved_status=True,
            leave_from__gte=first_day_of_month,
            leave_to__lte=last_day_of_month
        )

        # Calculate the total approved leave days
        total_approved_leave_days_month = approved_leaves.aggregate(Sum('total_days'))['total_days__sum'] or 0

        permited_leave_obj = LatePresentAndLeave.objects.latest('id') or None
        permited_leave = permited_leave_obj.allowed_leave or 0

        # # Totla Leaves in this year
        # approved_leave_days = LeaveApplication.objects.filter(
        #     Q(approved_status=True) &
        #     Q(leave_from__year=current_year)
        # ).aggregate(Sum('total_days'))

        # total_days_in_current_year = approved_leave_days.get('total_days__sum', 0)

        if total_approved_leave_days_month>permited_leave or applied_leave_days> permited_leave:
            messages.error(self.request, "You exceed your permitted Leave")
            return redirect(reverse('employee:apply_leave'))
